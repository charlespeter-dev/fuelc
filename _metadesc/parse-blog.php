...
$sql = "TRUNCATE _2x_metadesc_blog_xlsx";
$wpdb->query($sql);

foreach ($sheetData as $sd) {
    if (is_array($sd)) {
        if (isset($sd[0]) && $sd[0]) {
            if (preg_match('@^https?://fuelcycle.com/blog/([a-zA-Z0-9-_/]+)?$@i', $sd[0], $matches)) {

                $sql = "INSERT INTO _2x_metadesc_blog_xlsx SET url = '%s', new_meta_desc = '%s'";
...
$sql = "SELECT url, new_meta_desc FROM _2x_metadesc_blog_xlsx";
$res = $wpdb->get_results($sql, ARRAY_A);

foreach ($res as $xlsx) {
    $slug = str_replace(['https://fuelcycle.com/blog/', 'http://fuelcycle.com/blog/'], '', $xlsx['url']);
    ...
    if (!$slug) {
        ...
        if (isset($res_page['ID']) && $res_page['ID']) {
            ...
            $sql = "UPDATE _2x_metadesc_blog_xlsx SET old_meta_desc = '%s', post_id = %d, slug = '%s' WHERE url = '%s'";
...
// SELECT DISTINCT x.post_id, x.url, x.old_meta_desc, m.meta_value AS new_meta_desc FROM `_2x_metadesc_blog_xlsx` x LEFT JOIN wp_postmeta m ON m.post_id = x.post_id WHERE m.meta_key = '_yoast_wpseo_metadesc' ORDER BY `x`.`_2x_metadesc_blog_xlsx_id` ASC LIMIT 1000;
